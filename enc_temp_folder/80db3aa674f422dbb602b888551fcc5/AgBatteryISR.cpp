...
// TODO: plot first & second derivatives of charge curves ****
/// beware overnite temperature drop
...
bool report(int reportMinutes) {
	...
	batteryISR();
	printf("%.4f,%.4f,%.3f,%.0f,%.0f\n", vCell, vExternal2, isr, mAh, mWh);
...
bool charge() {
	...
	while (1) {
		...
		if (vExternal >= vPeak + 0.0005) {  // terminate very slow rise
			vPeak = vExternal;
			levelMins = 0;
